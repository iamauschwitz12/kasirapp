...
@once
    <!-- Modal Scanner Kamera OpnameGudang -->
    <x-filament::modal id="modal-scanner-opname-gudang" width="md" class="z-50">
        <x-slot name="heading">
            Scan Barcode Menggunakan Kamera
        </x-slot>

        <div class="flex flex-col items-center justify-center p-2">
            <div id="reader-opname-gudang"
        ...
        <x-slot name="footer">
            <div class="flex gap-3 pt-2">
                <x-filament::button type="button" color="gray" x-data
                    x-on:click="$dispatch('close-modal', { id: 'modal-scanner-opname-gudang' })" class="w-full">
                    Tutup Scanner
                </x-filament::button>
            </div>
        </x-slot>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.addEventListener('open-modal', (event) => {
                if (event.detail.id === 'modal-scanner-opname-gudang') {
                            ...
                            html5QrcodeScanner.render((decodedText, decodedResult) => {
                                ...
                                wire.call('scanBarcodeCamera', decodedText).then(product => {
                                    if (product) {
                                        ...
                                        // Set product_id — afterStateUpdated Filament otomatis mengisi field lainnya
                                        wire.set(productIdPath, product.id, true);

                                        // Tutup modal secara otomatis
                                        html5QrcodeScanner.clear().then(() => {
                                            html5QrcodeScanner = null;
                                            window.dispatchEvent(new CustomEvent('close-modal', { detail: { id: 'modal-scanner-opname-gudang' } }));
...
<div class="flex items-center justify-center pt-6">
    <x-filament::button type="button"
        x-on:click="$dispatch('open-modal', { id: 'modal-scanner-opname-gudang', statePath: '{{ $statePath }}' })"
